		...
		if (gpio_status & BIT(gpio_id)) {
			...
			if (i == 0) {
				if (single_key->level == 1) {// 5s, restart & enter softap mode
					...
					os_timer_setfn(&single_key->k_timer2,
							(os_timer_func_t *) key_50ms_cb, single_key);
